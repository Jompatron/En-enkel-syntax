...
def readGroup(q, z):
    ...
    if q.peek() == ".":
        ...
        z.store(q.dequeue())
        Ruta.down = readMolekyl(q, z)
        print("Här är ruta down " + str(readMolekyl(q, z)))
        if q.peek() != ")":
    ...
    if q.peek() in period:
        ...
        if q.peek() in small:
            if first + q.peek() in period:
                Ruta.atom = first + q.peek()
                readLetter(q)
                print("här är atom " + Ruta.atom)
                if q.peek() in number:
                    Ruta.num = q.peek()
                    readNum(q)
                    print("här är num " + Ruta.num)
            else:
                q.dequeue()
                raise Grammatikfel("Okänd atom vid radslutet ")
        elif q.peek() in number:
            Ruta.num = q.peek()
            readNum(q) # Denna siffra ska stå utanför parenteserna i grafiska
            print("här är num " + Ruta.num)
        else:
            return
